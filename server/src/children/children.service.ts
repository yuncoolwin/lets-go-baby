...
export class ChildrenService {
  ...
  async create(userId: string, dto: {
    ...
    await this.logAudit({ userId, action: 'child_create', targetType: 'child', targetId: data?.id || null, name: data?.name || null });
    return data;
  }
  ...
  async update(userId: string, id: string, dto: {
    ...
    if (dto.enrollment_duration && dto.start_date && !dto.end_date) {
      // 从数据库读取节假日数据
      const year = parseDate(dto.start_date).getUTCFullYear();
